...
function updateLogoSVG(variant) {
    console.log(`[Appearance] updateLogoSVG called with variant: ${variant} - delegating to logo.js`);
...
export function togglePalette(section) {
    ...
    // Fix: Use correct submenu ID for pins section
    ...
    console.log('🔍 Elements found:', {
        ...
        submenuId: submenuId,
        wrapperId: `palette-wrapper-${section}`
    });
    ...
    document.querySelectorAll('.palette-toggle-btn').forEach(b => b.classList.remove('active'));
...
export function updateSectionLayers(section, state) {
    ...
    if (section === 'logobg') {
        console.log(`[Appearance] Skipping logobg in updateSectionLayers - handled by color-utils`);
        return;
    }
...
export function updateFilter(section, hex) {
    // Skip logobg - it's handled by color-utils
    if (section === 'logobg') {
        console.log(`[Appearance] Skipping logobg in updateFilter - handled by color-utils`);
        return;
    }
    
    console.log(`[Appearance] updateFilter called with section: ${section}, hex: ${hex}`);
    
    const svg = document.querySelector('#svg-wrapper svg#svg8');
    console.log(`[Appearance] SVG found:`, !!svg);
    
    if (!svg) {
        console.warn(`[Appearance] SVG element not found`);
        return;
    }
    
    const rgb = hexToRgb(hex);
    console.log(`[Appearance] Converted RGB: ${rgb}`);
    
    let id = (section === 'logobg') ? 'feFlood-logobg-color' : `feFlood-${section}-color`;
    console.log(`[Appearance] Filter ID: ${id}`);
    
    const el = svg.querySelector(`#${id}`);
    console.log(`[Appearance] Filter element found:`, !!el);
    
    if (el) {
        el.setAttribute('flood-color', rgb);
        console.log(`[Appearance] Applied flood-color: ${rgb} to element: ${id}`);
    } else {
        console.warn(`[Appearance] Filter element not found: ${id}`);
    }
}
