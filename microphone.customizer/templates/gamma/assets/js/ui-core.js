...
import { togglePalette, handleStyleSelection, handleColorSelection, hexToRgbValues } from './modules/appearance-new.js';
...
const wrappedUpdateUI = function updateUI() {
    ...
    if (spheresSubtitle) {
        ...
        if (currentState.spheres.color) {
            ...
            // Use HL data to get variant name
            const hlData = stateManager.get('hlData');
            const spheresOptions = hlData?.options?.spheres || [];
            const currentVariant = spheresOptions.find(opt => opt.UF_VARIANT_CODE === currentState.spheres.variant);
            spheresSubtitle.textContent = currentVariant?.UF_VARIANT_NAME || 'Стандартный';
        }
    }
    ...
    if (bodySubtitle) {
        ...
        if (currentState.body.color) {
            ...
            // Use HL data to get variant name
            const hlData = stateManager.get('hlData');
            const bodyOptions = hlData?.options?.body || [];
            const currentVariant = bodyOptions.find(opt => opt.UF_VARIANT_CODE === currentState.body.variant);
            bodySubtitle.textContent = currentVariant?.UF_VARIANT_NAME || 'Стандартный';
        }
    }
    ...
    if (logoSubtitle) {
        ...
        logoSubtitle.textContent = currentState.logo.customLogo
            ...
            : (currentState.logo.variant === 'malfasilver' || currentState.logo.variant === 'malfagold'
                ? (currentState.logobg.color === '9006' ? 'MALFA Edition (Серебро)'
                    : (currentState.logobg.color === '9005' ? 'MALFA Edition (Золото)'
                        : 'MALFA Edition'))
    ...
    // Get HL data for shockmount color names
    const hlData = stateManager.get('hlData');
    let shockmountText = 'Белый'; // Default

    if (currentState.shockmount && currentState.shockmount.name) {
        // Use HL data name if available
        shockmountText = currentState.shockmount.name;
    } else if (currentState.shockmount && currentState.shockmount.variant) {
        ...
        shockmountText = shockmountColorNames[currentState.shockmount.variant] || 'Белый';
    }

    if (currentState.shockmount && currentState.shockmount.variant === 'ral' && currentState.shockmount.colorName) {
        shockmountText = currentState.shockmount.colorName;
    }
    ...
    // Shockmount Pins Subtitle using HL data
    let pinsText = 'RAL 1013'; // Default
    if (currentState.shockmountPins && currentState.shockmountPins.name) {
        pinsText = currentState.shockmountPins.name;
    }
    ...
    if (basePrice) {
        const currentModel = hlData?.models?.[hlData.currentModelId];
        const basePriceValue = currentModel?.UF_BASE_PRICE || 0;
    ...
    // Logo background price using HL data
...
export function initEventListeners() {
    ...
    document.querySelectorAll('.menu-item').forEach(item => {
        console.log('[initEventListeners] Setting up menu item:', item.dataset.section);
        item.addEventListener('click', function() {
        ...
        item.addEventListener('keydown', function(e) {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                console.log('[menu-item] Key pressed:', this.dataset.section);
                toggleSubmenu(this.dataset.section);
            }
        });
    });
    ...
    document.querySelectorAll('.submenu .variant-item').forEach(item => {
        const handler = function() {
            ...
            else if (section === 'shockmount' || section === 'pins') {
                ...
                if (zone.section === 'logo') {
                    import('./modules/logo.js').then(m => {
                        ...
                        reader.onload = event => {
                            ...
                            // Use HL data for logo pricing
                            const hlData = stateManager.get('hlData');
                            const logoPrice = hlData?.prices?.logo || 0;
                            setState('prices.logo', logoPrice);
...
export function handleShockmountColorSelection(color, ralName) {
    ...
    // Use HL data for shockmount pricing
    const hlData = stateManager.get('hlData');
    const shockmountPrice = hlData?.prices?.shockmount || 0;
    setState('prices.shockmount', shockmountPrice);
...
export function handleShockmountPinSelection(variant, color = null, ralName = null) {
    ...
    if (variant === 'custom') {
        ...
        const hlData = stateManager.get('hlData');
        const shockmountPinsPrice = hlData?.prices?.shockmountPins || 0;
        setState('prices.shockmount', isFree ? 0 : shockmountPinsPrice);
...
export function toggleSubmenu(section) {
    console.log('[toggleSubmenu] Called with section:', section);
    
    const menuItem = document.querySelector(`[data-section="${section}"]`);
    const submenu = document.getElementById(`submenu-${section}`);

    console.log('[toggleSubmenu] Found elements:', { menuItem: !!menuItem, submenu: !!submenu });

    if (!menuItem || !submenu) {
        console.warn('[toggleSubmenu] Elements not found for section:', section);
        return;
    }

    const isExpanded = menuItem.classList.contains('expanded');
    console.log('[toggleSubmenu] Current state:', { isExpanded });
    ...
    if (!isExpanded) {
        ...
        console.log('[toggleSubmenu] Submenu opened:', section);
