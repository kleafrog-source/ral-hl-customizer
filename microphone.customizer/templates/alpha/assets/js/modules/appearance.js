...
export function initPalettes() {
    ...
    sections.forEach(section => {
        ...
        for (let [id, colorData] of Object.entries(ralPalette)) {
            ...
            //создается для каждого RAL цвета в pal-{section}
            div.onclick = () => handleColorSelection(section, color, name);
            //обработчик клавиатуры для каждого RAL цвета в pal-{section}
            div.onkeydown = (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    //вызывает функцию handleColorSelection для каждого RAL цвета в pal-{section}
    ...
    if (logoBgContainer) {
        ...
        for (let [id, colorData] of Object.entries(ralPalette)) {
            ...
            div.onkeydown = (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    //вызывает функцию handleColorSelection для каждого RAL цвета в pal-logo
...
//функция обработки выбора цвета использует batch операции для предотвращения множественных рендеров.
//section - это название секции (pins, logobg, spheres, body, etc.), color в данной функции это HEX цвет, ralName - это название RAL цвета
export function handleColorSelection(section, color, ralName) {
    ...
    if (section === 'spheres' || section === 'body') {
        batchSet(`${section}.variant`, `${section}-ral-custom`);
        batchSet(`${section}.color`, ralLabel);
        batchSet(`${section}.colorValue`, color);
        // Цена будет рассчитана через price calculator

    } else if (section === 'logobg') {
        batchSet('logobg.variant', 'logobg-ral-custom');
...
export function updateSectionLayers(section, state) {
    ...
    // Task 4: Only apply color logic if custom color is selected AND variant is 4
    if (state.color && state.variant === '4') {
...
export function updateFilter(section, hex) {
    ...
    if (el) {
        ...
        console.log('[DEBUG flood-color appearance]', {
            filterId: id,
            section,
            rawColorValue: hex,
